...
class Compiler {
    ...
    resolverTransformer(transformer) {
        try {
            const value = utils_1.esmRequire(require.resolve(transformer, { paths: [this.appRoot] }));
    ...
    compileFile(filePath, contents, virtualFile) {
        utils_2.debug('compiling file using typescript "%s"', filePath);
    ...
    compile(filePath, contents, virtualFile = false) {
         ...
         */
        if (virtualFile) {
            utils_2.debug('compiling virtual file "%s" (no cache)', filePath);
            return this.compileFile(filePath, contents, true);
        }
        utils_2.debug('compiling file "%s"', filePath);
